...
export const fetchOrders = async (userId: string): Promise<Order[]> => {
  const withReturns = await supabase
    .from("orders")
    .select(
      "id, order_number, total_inr, total_amount, status, payment_status, payment_provider, payment_ref, refund_status, cancel_status, created_at, updated_at, shipping_address, shipments(id,carrier_name,tracking_number,awb_number,carrier_status,normalized_status,last_event_at,eta,tracking_url,shipment_events(id,event_time,raw_status,normalized_status,location)), order_items(id,order_id,product_id,variant_id,title_snapshot,price_inr,quantity,refund_attempts,exchange_attempts,refund_completed,exchange_completed,refund_locked,exchange_locked,active_request,refund_allowed_override,exchange_allowed_override,manual_override_reason,manual_override_admin_id,manual_override_at,product:products(id,title,image_url,return_allowed,exchange_allowed,return_window_days,product_variants(id,color,size,active)),variant:product_variants(id,color,size)), return_requests(id,order_id,order_item_id,product_id,user_id,exchange_variant_id,type,reason,description,photos,status,pickup_status,pickup_awb,pickup_tracking_number,pickup_tracking_url,refund_id,refund_status,refund_amount_inr,refunded_at,admin_note,admin_user_id,created_at,updated_at,return_events(id,event_type,message,payload,created_at))"
    )
    .eq("user_id", userId)
  ...
  if (withReturns.error) {
    const withoutCancel = await supabase
      .from("orders")
      .select(
        "id, order_number, total_inr, total_amount, status, payment_status, payment_provider, payment_ref, refund_status, created_at, updated_at, shipping_address, shipments(id,carrier_name,tracking_number,awb_number,carrier_status,normalized_status,last_event_at,eta,tracking_url,shipment_events(id,event_time,raw_status,normalized_status,location)), order_items(id,order_id,product_id,variant_id,title_snapshot,price_inr,quantity,product:products(id,title,image_url),variant:product_variants(id,color,size))"
      )
      .eq("user_id", userId)
      .order("created_at", { ascending: false });
